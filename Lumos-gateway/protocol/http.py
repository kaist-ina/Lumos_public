...
# Code by Jeongmin Kim *Start*
#############################################################################################

#############################################################################################
# Code by Jeongmin Kim *End*
    ...
    def __call__(self):
        ...
        while True:
            ...
            try:
                ...
                # Code by Jeongmin *Start*
                # Code for Lumos
                #############################################################################

                #############################################################################
                # Start dynamic learner
                #############################################################################
                from . import dynamic_learner
                # packet_type = dynamic_learner.DynamicLearner.check_packet_types(flow)
                #
                # if packet_type is 0:
                #     #start recording
                #     pass
                # elif packet_type is 1:
                #     #end recording
                #     pass
                # elif packet_type is 2:
                #     #interaction notification
                #     pass
                #

                # in this point, start_learning method emits targetpair for this flow.
                # we need to save this flow after response receive.
                targetpair = dynamic_learner.DynamicLearner.start_learning(flow)


                ############################################################################
                # End of dynamic learner
                ############################################################################


                # Start Interoperation manager.
                # Interoperation manager does run continually.

                from . import interoperation_manager

                # load interops
                if os.path.exists(interoperation_manager.InteroperationManager._path + "/interops.bin"):
                    interoperation_manager.InteroperationManager.interoperation_conf_load(flow)
                #
                # # perform interoperation
                interoperation_manager.InteroperationManager.interoperation_runner(flow)
                ...
                if not flow.response:
                    ...
                    # when targetpair is only exist,
                    if targetpair is not None:
                        targetpair.actualpacket.append(flow.get_state())
                else:
