...
//		forwardSolver.setEnableMergePointChecking(true);
        ...
        /**
         * Backward analysis seed setting for UI finder.
         */
		// If this phase is for UIfinder, we need to add taint seeds manually.
        if (EHSigManager.ObtainedSeeds.size() > 0) {
            backProblem.initialSeeds().clear();
            sinkCount += addSeedsforUI(backProblem);
        }
    ...
    private int addSeedsforUI(MyBackwardsInfoflowProblem backwardProblem) {
        int i = 0;
	    for (EPSeedPair eps : EHSigManager.ObtainedSeeds)
        {
            Unit u = eps.getSeed();
            SootMethod m = eps.getSeedParentMethod();
            i++;
    ...
    private void GraphTravler(IInfoflowCFG iCfg) {
		System.out.println("EntryPoints!!");

		SootClass sc = Scene.v().getSootClass("com.philips.lighting.hue2.DialogActivity");
		for (SootMethod sm : sc.getMethods())
		{
			if (sm.getSignature().contains("void a(android.content.Context,com.philips.lighting.hue2.fragment.BaseFragment)"))
			{
				for (Unit ut : sm.getActiveBody().getUnits())
				{
					if (ut.toString().contains("<android.content.Context: void startActivity(android.content.Intent)>"))
					{
						System.out.println(ut.toString());
						System.out.println("Units");
						System.out.println(iCfg.getCalleesOfCallAt(ut));
					}
				}
			}
		}
...
//		{
//			if (sc.getName().contains("Dummy"))
//			{
//				System.out.println(sc.getName());
//			}
//		}
//		for (SootMethod sm : Scene.v().getSootClass("com.philips.lighting.hue2.ContentActivity").getMethods())
//		{
			...
			for(Unit ut : Scene.v().getMethod(sft.getSourceMethod()).getActiveBody().getUnits()){
				...
				if (ut.toString().equals(sft.getUnit().toString()))
				{
					...
					if(ut instanceof AssignStmt){
						...
						if (getConfig().getLogSourcesAndSinks())
							collectedSources.add(s);
						System.out.println(iCfg.getMethodOf(ut));
