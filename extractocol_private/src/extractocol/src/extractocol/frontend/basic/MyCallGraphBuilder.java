...
import extractocol.common.UIfinder.ListenerEntry;
...
public class MyCallGraphBuilder {
	 ...
	 * Editor: JM
	 * Edit purpose: to find exact call site of doInbackground, and run methods.
	 */
			...
			for(Iterator<SootMethod> smIter = sc.getMethods().iterator(); smIter.hasNext();){
				...
				for(Unit u : sm.getActiveBody().getUnits()){
					...
					if(!implicitCall(sm, s)){
						if(isSystemPackage(s) && !s.getInvokeExpr().getMethod().getName().endsWith("Listener"))
							continue;
	 ...
	 * Author: BK
	 * Editor: Jm to find exact run, doInbackground() caller
	 */
	...
	private static boolean implicitCall(SootMethod srcSM, Unit u){
		...
		final NumberedString sigexecuteOnExecutor = Scene.v().getSubSigNumberer().findOrAdd("android.os.AsyncTask executeOnExecutor(java.util.concurrent.Executor,java.lang.Object[])");
		final NumberedString sigdoInBackground = Scene.v().getSubSigNumberer().findOrAdd("java.lang.Object doInBackground(java.lang.Object[])");
		...
		if (ie instanceof InstanceInvokeExpr) {
			...
			else if (subSig == sigStart)
			{
				Value runnable = iie.getBase();
				SootClass sc = Scene.v().getSootClass(runnable.getType().toString());
				SootMethod targetSM = sc.getMethodUnsafe(sigRun);
			...
			else if (subSig == sigexecuteOnExecutor)
			{
				Value runnable = iie.getBase();
				SootClass sc = Scene.v().getSootClass(runnable.getType().toString());
				SootMethod targetSM = sc.getMethodUnsafe(sigdoInBackground);
				...
				if(isValidMethod(targetSM))
					...
					for (SootClass superClass : superClasses) {
						targetSM = superClass.getMethodUnsafe(sigdoInBackground);
	...
	private static void VirtualInvokeHandler(SootMethod srcSM, Stmt s) {
        ...
        /**
         * In progress
         * Finding setOnXXListeners
         * Author: JM
         */
        if (Constants.FindingEventHandler)
        {
            if (ie.getMethod().getName().startsWith("setOn") &&
                    ie.getMethod().getParameterCount() == 1 &&
                    ie.getMethod().getParameterTypes().get(0).toString().endsWith("Listener"))
                EHSigManager.addListener(s, (VirtualInvokeExpr)ie, srcSM);
        }
 	}
