...
public class Frontend {
	...
	public static void mainForUI(String[] args)
    {
        // 1. Argument parsing
        if(!Argument_Parsing(args)){
            System.err.println("Argument is weird.");
...
//        postProcessing();
    ...
    private static void MergingResultsForUI() {
        // Get the temp directory
        String path = Constants.getPartialDPListDirPath();
	...
	private static void getStmtHashforUI() {
        ExtractocolLogger.Log("Calculating hash value for target stmt ...");

        for (EPSeedPair eps : EHSigManager.ObtainedSeeds) {

            int hash = MyCallGraphBuilder.getUnitHash(eps.getSeed(), eps.getSeedParentMethod());
            DPandHASH.add(new Pair2<String, Integer>(eps.getEP(), hash));
            ExtractocolLogger.Log(eps.getSeed().toString() + " [Hash: " + hash + "] in " + eps.getSeedParentMethod().toString());
