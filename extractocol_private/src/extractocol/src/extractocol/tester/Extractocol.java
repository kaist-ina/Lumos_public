...
public class Extractocol {
    ...
    public static void main(String[] args) {
        ...
        // 4. Frontend - we do not need frontend because frontend will be started in backend phase in case of UI finding.
...
//
        ...
        // 7. Forward Taint for UI finding
//		FrontendForUIFinding.main(getFrontendArg());
...
//		System.exit(0);
    ...
    protected static void Extractocol_Frontend() {
        long start, end;
    ...
    /***                    Get arguments                 ***/
    /********************************************************/
    private static String[] getRetrofitArg() {
        return splitter(Constants.getAPPName() + (debug ? " --debug" : ""));
    }

    private static String[] getFrontendArg() {
        return splitter(Constants.getAPPName());
    }

    private static String[] getBackendArg() {
        return splitter("--app " + Constants.getAPPName());
    }

    private static String[] getHeapHandlingArg() {
        return splitter("--app " + Constants.getAPPName());
    }
    ...
    private static boolean ArgParsing(String[] args) {
        ...
        while (k < args.length) {
            ...
            // set UI finding
            else if (args[k].equalsIgnoreCase("--UI")) {
                Constants.FindingEventHandler = true;
                k++;
                continue;
            }
    ...
    public static void Extractocol_Frontend_forUI() {
        long start, end;

        System.out.println("Frontend for UI finding running ...");

        start = System.currentTimeMillis();
        if (!debug) StdOutputController.stop();
        Frontend.mainForUI(getFrontendArg());
        if (!debug) StdOutputController.start();
        end = System.currentTimeMillis();
