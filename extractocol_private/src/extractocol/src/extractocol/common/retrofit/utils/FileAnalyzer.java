...
public class FileAnalyzer {
	...
	public static void printLine(String line, int tabNum, int lineNum, String path) {
		// JM modified
//		if(debug) {
//			LineType lt = lineTypeChecker(line, currentTran);
//			String tab = "";
...
//		}
	 ...
	 * @return line type
	 */
	public static LineType lineTypeChecker(String line, Transaction currentTran, List<String> importList) {
		...
		else if (currentTran != null && isMethodPrototype(line, importList)) {
	...
	public static boolean isMethodPrototype(String line, List<String> importList) {

		/**********************************/
		/* add Import filter because of this method might be wrong in line 154 */
		boolean isRetrofit = false;
		for (String imp : importList)
		{
			if (imp.contains("retrofit"))
				isRetrofit = true;
		}
		...
		if(!(line.matches("^  [^ ].+") && line.contains("(") && line.endsWith(";") && !line.contains("=")) // FilpagramService.java (Flipagram)
				&& !(line.matches("^    [^ ].+") && line.contains("(") && line.endsWith(";") && !line.startsWith("    return") && !line.contains("="))
				&& !(line.matches("^    [^ ].+") && line.contains("(") && line.endsWith(";") && !line.startsWith("    return")))
				// ConsumerApi.java (DoorDash) -> This is not appliable to SmartThings
			return false;
