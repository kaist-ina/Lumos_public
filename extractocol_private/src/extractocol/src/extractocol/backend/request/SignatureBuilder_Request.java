	...
	public void DetermineJumpOtherMethod(int invokeType, InvokeExpr ie, HashMap<String, ArrayList<BFNode>> BFTtable, Unit ut, ParameterBucket pb)
	{
		try
		{
			...
			if (SemanticModel.contains(Constants.Deobfuse(ie.getMethodRef().toString())) || ie.getMethodRef().toString().contains("<com.squareup.okhttp.")
                    ...
                    || ie.getMethodRef().toString().contains("android.view.View findViewById(int)>")
                    || ie.getMethodRef().toString().contains("setOnClickListener(android.view.View$OnClickListener)")
                    || ie.getMethodRef().toString().contains("setOnTouchListener(android.view.View$OnTouchListener)")
                    || ie.getMethodRef().toString().contains("setOnSeekBarChangeListener(android.widget.SeekBar$OnSeekBarChangeListener)")
                    || ie.getMethodRef().toString().contains("setOnTouchListener(android.view.View$OnTouchListener)>"))
				isSemantic = true;
