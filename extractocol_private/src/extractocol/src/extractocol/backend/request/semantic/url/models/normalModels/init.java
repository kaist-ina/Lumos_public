...
//			BackendOutput.reqRespInfo.setRequestBody(map);
		...
		//JM
		else if (method.equals("<com.netflix.mediaclient.service.logging.customerevents.legacy.MdxCustomerEvent: void <init>(java.lang.String,com.netflix.mediaclient.webapi.AuthorizationCredentials,java.lang.String)>"))
		{
			spb.CurrentPB.BT().RRI().SaveURI("https://customerevents.netflix.com/.*");
    		String[] KVpairs = spb.CurrentPB.varTable.getValueEntryList(spb.iie.getArg(2).toString()).GenRegex().split("###");
    		for (String pair : KVpairs)
            {
                String[] kv = pair.split("#:#");
                if (!kv[0].equals("") && kv.length == 2)
                    spb.CurrentPB.BT().RRI().addRequestBody(kv[0], kv[1]);
            }
		}
		// JM
		else if (method.equals("<com.obsidian.v4.data.cz.service.z: void <init>(java.lang.String,com.obsidian.v4.data.cz.service.NetRequest$RequestMethod)>"))
        {
            spb.CurrentPB.BT().RRI().SaveURI (spb.CurrentPB.varTable.getValueEntryList(spb.iie.getArg(0).toString()).GenRegex());
        }
        // JM
		else if (method.equals("<com.logitech.harmonyhub.sdk.Request: void <init>(java.lang.String,int)>"))
		{
			if (spb.iie.getArg(0) instanceof Constant) {
				spb.CurrentPB.BT().RRI().addRequestBody("requestType", spb.iie.getArg(0).toString());
